...
if __name__ == '__main__':
    ...
    loader_train = DataLoader(dataset_train, batch_size=4096, shuffle=False, sampler=train_sampler)
    verif_sampler = DistributedSampler(dataset_train, num_replicas=hvd.size(), rank=hvd.rank())
    loader_verif = DataLoader(dataset_train, batch_size=4096, shuffle=False, sampler=verif_sampler)    
    ...
    optimizer = optim.Adam(PPLSTM_model.parameters(), lr=0.00005* hvd.size(), weight_decay=1e-5)
    optimizer = hvd.DistributedOptimizer(optimizer, named_parameters=PPLSTM_model.named_parameters())

    hvd.broadcast_parameters(PPLSTM_model.state_dict(), root_rank=0)
