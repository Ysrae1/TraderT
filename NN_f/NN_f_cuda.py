...
if __name__ == '__main__':
    ...
    dataset_train_dn = H5Dataset(f'{data_dir}lob_data_train_f{seq_len_forward}.h5','d')

    dataset_verif = H5Dataset(f'{data_dir}lob_data_verif_f{seq_len_forward}.h5','n')
    dataset_verif_dn = H5Dataset(f'{data_dir}lob_data_verif_f{seq_len_forward}.h5','d')

    dataset_test = H5Dataset(f'{data_dir}lob_data_test_f{seq_len_forward}.h5','n')
    dataset_test_dn = H5Dataset(f'{data_dir}lob_data_test_f{seq_len_forward}.h5','d')
    ...
    if torch.cuda.device_count() > 1:
        PPLSTM_model = nn.DataParallel(PPLSTM_model)
