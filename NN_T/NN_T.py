...
class PP_Transformer(nn.Module):
    def __init__(self, seq_len_f, feature_size=14, num_layers=1, num_heads=16):
        super(PP_Transformer, self).__init__()
        self.hidden_dim = 10
        self.seq_len_f = seq_len_f
        ...
        self.embedding = nn.Linear(feature_size, self.hidden_dim)
        
        self.dense_1 = nn.Linear(self.hidden_dim, 5)
        self.dense_2 = nn.Linear(5, 3)
        self.dense_3 = nn.Linear(3, seq_len_f)
        self.relu = nn.ReLU()

    def forward(self, x):
        # Embedding + Positional Encoding (if needed)
        x = self.embedding(x)
        
        # Transformer Encoder
        x = self.transformer_encoder(x)
        
        # We take the output from the last token in the sequence
        x = x[:, -1, :]
        
        # Sequential dense layers with ReLU activations
        x = self.relu(self.dense_1(x))
        x = self.relu(self.dense_2(x))
        x = self.dense_3(x)
